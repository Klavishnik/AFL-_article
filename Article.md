...
На всем протяжении статьи Вам будут встречаться ссылки на исходный код. Они ведут на последнюю стабильную версию AFL++ 4.30. Все, что описывается в этой статье, основывается именно на этой версии, но с большой долей вероятности будет актуально, как для версий ниже, так и для версий ниже, поскольку затрагивает макисмально базовые механизмы работы данного фаззера.
...
`_afl_area_ptr` - это массив, к которому осуществляется доступ каждый раз при достижении новой области. Обратите внимание, что есть доступ к `__afl_area_ptr` происходит во всех ветвелниях кода  - в операторе if и в операторе else.
...
Метки `dword_91A0` и `dword_919C` IDA PRO явяеются указателями на участок памяти и относятся к массиву нашей карты памяти, что видно далее:
...
- `__afl_area_initial` — Карта покрытия, используемая до отображения общей памяти, а также если общая память недоступна (например, если программа не выполняется под управлением `AFL++`). Эта карта создается как массив в файле `afl-compiler-rt.o.c`.
...
- `__AFL_SHM_FUZZ_ID` ([алиас](https://github.com/AFLplusplus/AFLplusplus/blob/v4.30c/include/config.h#L409) `SHM_FUZZ_ENV_VAR`) — Идентификатор общей памяти для fuzzing через общую память.
- `AFL_MAP_SIZE` — Используется для задания размера буфера общей памяти, выделяемого afl-fuzz.

**Макросы:**

- `MAP_SIZE` — Пользовательское значение, которое `afl-fuzz` будет использовать для задания размера карты общей памяти. изначально [зависит от системы](https://github.com/AFLplusplus/AFLplusplus/blob/v4.30c/include/config.h#L405)
...
1. `__afl_auto_first` - она по сути [ничего не делает](https://github.com/AFLplusplus/AFLplusplus/blob/v4.30c/instrumentation/afl-compiler-rt.o.c#L1337), только устанавливает `__afl_already_initialized_first = 1`, чтобы показать, что процесс инициализации был и проверяет наличие LLVM инструментации.
...
- Служат основой для реализации карты покрытия.
