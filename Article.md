# От автора 
Написание этой статьи было вызвано отсутствием комплексных материалов о структуре фаззера AFL++ на русском языке, а также необходимостью устранить собственные пробелы в знаниях.
...
Получить доступ к этим мнструментациям можно с помощью базовых компиляторов AFL++:
- `PCGUARD` инструментация предоставляется компилятором `afl-clang-fast`.
- `LTO` инструментация предоставляется компилятором `afl-clang-lto`.
...
Недооценёеной проблемой оригинального AFL были коллизии при заполнении карты покрытия. Эта проблема была сильно недооценена сообществом и приводила к потерям покрытия и снижению эффективности фаззинга.

В AFL++ с этим долго боролись и смогли частично решить эту проблему лишь в LLVM 9 версии, где появилась функция `LLVM: SplitEdge()` - с помощью неё можно было вставлять блоки кода для инструментации на каждом ребре (`edge`) и инкрементировать счетчик в карте покрытия (об этом ниже).
...
Начиная с версии 12+ появилась возможность реализации подмены компановщика, что позволило обойти все проблемы, связанные с коллизиями.
...
Наглядно видно, что макросы AFL++ разворачиваются в достаточно простые команды, которые создают статический массив для входных данных 
`fuzz_buf`, считывают в него данные функцией `read` и записывают длину полученного массива в `fuzz_len`. 
