...
Для работы AFL++ бинарный файл должен быть инструментирован спецальными инструкциями. Это необходимо для того, чтобы фаззер мог отслеживать открытие новых ребер с помощью мутированных данных при фаззинг тестировании. Это и назыается покрытием кода. Такие инструментации могут быть как динмаческими, так и  статическими. В рамках данной статьи будем обсуждать статический способо иснтрументирования кода с помощью компиляторов AFL++. 
...
> Видно, что основная работа с разделяемой памятью происходит в функции shmat. Подробнее можно прочитать [здесь](https://www.opennet.ru/man.shtml?topic=shmat&category=2&russian=0)
